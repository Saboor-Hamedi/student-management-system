$(document).ready(function () {
    ...
    var selectedTeacherIdField = $('#student_id');
    // Event listener for input changes
    $('#search_student_names').on('input', function () {
        var query = $(this).val().trim();

        if (query !== '') {
            // Make an AJAX call to the PHP file to get the search results
            $.ajax({
                url: '../../views/files/Loadstudents.php',
                method: 'POST',
                data: { roles_id: query },
                dataType: 'json',
                cache: false,
                success: function (data) {
                    ...
                    if (data.length > 0) {
                        ...
                        data.forEach(function (teacher, index) {
                            ...
                            html += '<li data-student_id="' + teacher.id + '">' + teacher.username + ' (ID: ' + teacher.id + ')</li>';
                        });
    ...
    $('#search-results2').on('click', 'li', function () {
        var teacherId = $(this).data('student_id');
        var teacherName = $(this).text().split(' (ID:')[0]; // Extract the teacher name from the list item text
        $('#search_student_names').val(teacherName);
        selectedTeacherIdField.val(teacherId);
        $('#search_student_names').focus();
        if (div !== null) {
            div.style.display = "none";
        }
    });
    ...
    $(document).on('click', function (e) {
        if (!$(e.target).closest('#search-results2').length) {
            if (div !== null) {
                div.style.display = "none";
            }
        }
